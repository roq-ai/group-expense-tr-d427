...
import { useOrganizationFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'administrator', 'member'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useOrganizationFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Organizations',
      path: isTenantUser ? `/organizations/view/${data?.id}` : '/organizations',
      entity: 'organization',
      service: AccessServiceEnum.PROJECT,
      icon: FiBriefcase,
    },
    { name: 'Expenses', path: '/expenses', entity: 'expense', service: AccessServiceEnum.PROJECT, icon: FiCreditCard },
    { name: 'Groups', path: '/groups', entity: 'group', service: AccessServiceEnum.PROJECT, icon: FiUsers },
    {
      name: 'Memberships',
      path: '/memberships',
      entity: 'membership',
    ...
    { name: 'Payments', path: '/payments', entity: 'payment', service: AccessServiceEnum.PROJECT, icon: FiDollarSign },
