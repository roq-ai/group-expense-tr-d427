...
import { useRoqClient, useGroupFindFirst } from 'lib/roq';
...
function GroupEditPage() {
  ...
  const { data, error, isLoading, mutate } = useGroupFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: GroupInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.group.update({
        data: values as RoqTypes.group,
        where: {
          id,
        },
      });
