...
import { useGroupFindFirst, useRoqClient } from 'lib/roq';

import { GroupInterface } from 'interfaces/group';
import { MembershipListPage } from 'pages/memberships';
...
function GroupViewPage() {
  ...
  const queryParams = useMemo<{ include: { organization: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useGroupFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('group', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/groups/edit/${id}`} passHref legacyBehavior>
                    <Button
