...
import { useRoqPlatformUserProfiles, usePaymentCount, useGroupCount, useExpenseCount } from 'lib/roq';

import { MembershipListPage as ShowCaseList } from 'pages/memberships';

function DashboardPage() {
  const { session } = useSession();

  const paymentStat = usePaymentCount();

  const groupStat = useGroupCount();

  const expenseStat = useExpenseCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Payments',
      result: paymentStat?.data,
      isLoading: paymentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Groups',
      result: groupStat?.data,
      isLoading: groupStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Expenses',
      result: expenseStat?.data,
      isLoading: expenseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`memberships`} passHref legacyBehavior>
                  <Button
