...
import { useRoqClient, useOrganizationFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<OrganizationInterface, unknown>;
...
export function OrganizationListPage(props: OrganizationListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'expense.count', 'group.count'],
  ...
  const { data, error, isLoading, mutate } = useOrganizationFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.organization.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: OrganizationInterface) => {
    if (hasAccess('organization', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/organizations/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('expense', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Expense',
          accessorKey: 'expense',
          cell: ({ row: { original: record } }: any) => record?._count?.expense,
        }
      : null,
    hasAccess('group', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Group',
          accessorKey: 'group',
          cell: ({ row: { original: record } }: any) => record?._count?.group,
        }
      : null,
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="organization" />
        </Flex>
      </Flex>
      {showSearchFilter && (
        <Flex
