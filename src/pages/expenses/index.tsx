...
import { useRoqClient, useExpenseFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<ExpenseInterface, unknown>;
...
export function ExpenseListPage(props: ExpenseListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'organization', 'payment.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['description.contains'],
          ...params.filters,
        },
        'expense',
      ),
  ...
  const { data, error, isLoading, mutate } = useExpenseFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.expense.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: ExpenseInterface) => {
    if (hasAccess('expense', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('payment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Payment',
          accessorKey: 'payment',
          cell: ({ row: { original: record } }: any) => record?._count?.payment,
        }
      : null,
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="expense" />
        </Flex>

        {hasAccess('expense', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/expenses/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
