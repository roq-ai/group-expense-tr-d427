...
import { useRoqClient, useExpenseFindFirst } from 'lib/roq';
...
function ExpenseEditPage() {
  ...
  const { data, error, isLoading, mutate } = useExpenseFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ExpenseInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.expense.update({
        data: values as RoqTypes.expense,
        where: {
          id,
        },
      });
