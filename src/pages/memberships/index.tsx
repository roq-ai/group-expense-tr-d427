...
import { useRoqClient, useMembershipFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<MembershipInterface, unknown>;
...
export function MembershipListPage(props: MembershipListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useMembershipFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.membership.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: MembershipInterface) => {
    if (hasAccess('membership', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/memberships/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) => (record?.joined_at ? format(record?.joined_at, 'dd-MM-yyyy') : ''),
    },
    {
      id: 'left_at',
      header: 'Left At',
      accessorKey: 'left_at',
      cell: ({ row: { original: record } }: any) => (record?.left_at ? format(record?.left_at, 'dd-MM-yyyy') : ''),
    },
    ...
    hasAccess('group', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'group',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/groups/view/${record.group?.id}`}>
              {record.group?.name}
            </Link>
          ),
        }
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/memberships/view/${record.id}`} passHref legacyBehavior>
                <Button
