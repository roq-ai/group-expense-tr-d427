...
import { useMembershipFindFirst, useRoqClient } from 'lib/roq';
...
function MembershipViewPage() {
  ...
  const queryParams = useMemo<{ include: { user: boolean; group: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useMembershipFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('membership', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/memberships/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Joined At" text={data?.joined_at ? format(data?.joined_at, 'dd-MM-yyyy') : ''} />

                <FormListItem label="Left At" text={data?.left_at ? format(data?.left_at, 'dd-MM-yyyy') : ''} />
                ...
                {hasAccess('group', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Group"
                    text={
                      <Link as={NextLink} href={`/groups/view/${data?.group?.id}`}>
                        {data?.group?.name + ''}
                      </Link>
                    }
                  />
