...
import { useRoqClient, useMembershipFindFirst } from 'lib/roq';
...
function MembershipEditPage() {
  ...
  const { data, error, isLoading, mutate } = useMembershipFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: MembershipInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.membership.update({
        data: values as RoqTypes.membership,
        where: {
          id,
        },
      });
